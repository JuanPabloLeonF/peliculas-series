...
@Component({
  ...
  styleUrl: './register-series.component.css'
})
export class RegisterSeriesComponent {
  @Output() closePanel = new EventEmitter<boolean>();
  @Input() idSeriesSelected?: string;
  ...
  private createSeries(): void {
    if (this.idSeriesSelected) {
      this.createSeasons(this.idSeriesSelected);
    } else {
      const formData = new FormData();

      formData.append("category", JSON.stringify(this.formularyRegisterData.get('category')?.value))

      this._apiSeriesService.create(formData).subscribe((data: any) => {
        this.createSeasons(data.id);
      }, (error: any) => {
  ...
  private async createSeasons(seriesId: string) {
    const formData = new FormData();
    ...
    this._apiSeriesService.createSeasons(formData).subscribe((data: any) => {
      this.seasonsId = data.id;
    }, (error: any) => {
      console.error('Error al crear la seasons:', error);
    });
  }

  public onCreateVideoByChapter(selectedIndex: number): void {
    const formData = new FormData();
  ...
  private createVideos(): void {
    ...
    this._apiSeriesService.createVideos(formData).subscribe((data: any) => {
      ...
      this.stringFileImageChapter = "PORTADA CAPITULO";
      this.stringFileVideoChapter = "VIDEO CAPITULO";
      alert("SE HA CREADO EXITOSAMENTE LA SERIE")
  ...
  public onContinue(): void {
    if (this.formularyRegisterData.invalid) {
      ...
      const errors = [];
      if (this.idSeriesSelected) {
        fieldsToCheck = ['title', 'date', 'rating', 'description'];
      } else {
        fieldsToCheck = ['title', 'category', 'date', 'rating', 'description'];
      }
